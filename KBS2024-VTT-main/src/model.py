...
class build_model():
    ...
    def train(self, train_loader, valid_loader, test_loader, alpha=.5, beta=.5):
        ...
        if isinstance(self.model, nn.DataParallel):
            self.model.module.load_state_dict(torch.load(best_model_path, weights_only=False)['weight'])
        else:
            self.model.load_state_dict(torch.load(best_model_path, weights_only=False)['weight'])
